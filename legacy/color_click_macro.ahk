...
; 감지할 색상 목록 - 여기에 계속 추가하면 됨 (F8로 하나씩 뽑아서 콤마로 추가)
TargetColors := [
    0xD1CAF9,   ; 연보라  (플로어 사첵)
    0xB0E1AD,   ; 연두/연한 초록(밋그석)
    0xC9A5D7,    ; 진보라(1루 블루석)
    0xAEE5FF    ; 연하늘색(1루 테이블석, 일반석)
]
Variation     := 20        ; 색상 오차 허용치(0~255). 목록의 각 색 기준 R/G/B 채널별 ±45까지 허용.
                             ; 색이 계속 흔들려서 안 잡히면 더 올리고, 엉뚱한 곳에서 반응하면 줄이세요.
...
ShowConfig() {
    global TargetColors, Variation, SearchX1, SearchY1, SearchX2, SearchY2
...
PickColor() {
    ...
    ToolTip "커서 위치 색상: " c "  (X=" mx ",Y=" my ")`n이 값을 TargetColors 목록에 콤마로 추가하세요"
    SetTimer () => ToolTip(), -4000
}

; TargetColors 목록을 순서대로 검색해서 하나라도 걸리면 즉시 반환
FindTarget(&fx, &fy) {
    global SearchX1, SearchY1, SearchX2, SearchY2, TargetColors, Variation
    for c in TargetColors {
        if PixelSearch(&fx, &fy, SearchX1, SearchY1, SearchX2, SearchY2, c, Variation)
            return true
    }
    return false
}
...
RunLoop() {
    global Running, Bx, By, PollDelay, CooldownAfterClick, OffsetX, OffsetY
